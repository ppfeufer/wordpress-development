 ...
 */
class DebugUtility {
    const MARKER_KEY = '_DebugUtility_';
     ...
     * Constructor
     *
     * @param string|Formatter $format Output format ID, or formatter instance defaults to 'html'
     */
    public function __construct($format = 'html') {
        ...
        $this->fmt = $format;
        if(!\is_a($this->fmt, '\WordPress\Utility\Formatter')) {
            $format = isset(static::$config['formatters'][$format]) ? static::$config['formatters'][$format] : '\WordPress\Utility\\' . \ucfirst($format) . 'Formatter';
    ...
    static function debug() {
        ...
        if(!\is_countable($expressions) || \func_num_args() !== \count($expressions)) {
            $expressions = null;
        }
        ...
        if(!$capture && ($format === 'html') && !\headers_sent() && (!\ob_get_level() || \ini_get('output_buffering'))) {
            print '<!DOCTYPE HTML><html><head><title>DEBUG</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /></head><body>';
        }

        $ref = new DebugUtility($format);
                ...
                foreach(\array_slice($tokens, $i + 2) as $token) {
                    ...
                    if($lvl < 0) {
                        foreach($expressions as &$expression) {
                            $expression = \trim(\implode('', $expression));
                        }
    ...
    protected function linkify(\Reflector $reflector, $constant = null) {
        ...
        if($parent->isInternal()) {
            ...
            $valid = (($type === 'method') || (\strcasecmp($parent->name, 'stdClass') !== 0)) && (($type !== 'method') || (($reflector->name === '__construct') || \strpos($reflector->name, '__') !== 0));
    ...
    protected function evaluate(&$subject, $specialStr = false) {
        switch($type = \gettype($subject)) {
            ...
            case 'string':
                ...
                if(static::$config['showStringMatches'] && ($length > 2) && (\trim($subject) !== '')) {
                    ...
                    if(!$isNumeric && ($length > 4)) {
                        ...
                        $isSerialized = ($unserializing < 3) && (($subject[$length - 1] === ';') || ($subject[$length - 1] === '}')) && \in_array($subject[0], ['s', 'a', 'O'], true) && ((($subject[0] === 's') && ($subject[$length - 2] !== '"')) || \preg_match("/^{$subject[0]}:[0-9]+:/s", $subject)) && (($unserialized = \unserialize($subject)) !== false);
